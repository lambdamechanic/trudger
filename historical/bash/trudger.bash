...
while true; do
  ...
  while true; do
    ...
    solve_args=()
    if ((review_loops > 0)); then
      solve_args=("${review_args[@]}")
    fi
    if ! run_agent_command "$agent_command" "agent_solve" "$prompt_trudge_content" "" "${solve_args[@]}"; then
    ...
    if ((review_loops < review_loop_limit)); then
      log_transition "review_loop_retry task=${task_id} loop=${review_loops} limit=${review_loop_limit}"
      continue
    fi

    log_transition "review_loop_exhausted task=${task_id} loops=${review_loops} limit=${review_loop_limit}"
    run_task_command "$task_update_in_progress_command" "$task_id" --status blocked
    set_task_status_state "blocked"
    needs_human_tasks+=("$task_id")
    log_transition "needs_human task=${task_id}"
    run_hook "$hook_on_requires_human" "$task_id" "on_requires_human"
    break
