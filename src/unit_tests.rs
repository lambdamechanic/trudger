...
use crate::app::{
    main_with_args, render_prompt, require_file, run_with_args, run_with_cli, run_with_cli_for_test,
};
...
#[test]
fn run_with_cli_emits_task_end_once_when_on_completed_hook_fails() {
    let _guard = ENV_MUTEX.lock().unwrap();
    reset_test_env();
...
commands:
  ...
  task_show: "printf 'show\n'"
  task_status: "if [ -f '{marker_path}' ]; then printf 'closed\n'; else touch '{marker_path}'; printf 'ready\n'; fi"
  task_update_status: "true"
review_loop_limit: 2
hooks:
  on_completed: "exit 9"
...
"#,
    ...
    .expect_err("expected on_completed failure");
    assert_eq!(err.code, 1);
    ...
    assert_eq!(
        ...
        1,
        "task_end should be emitted once when on_completed fails, got:\n{order}"
    );
