## Context
We want a minimal shell script that automates a br + Codex loop for tasks labeled `trudgeable`. The script should start/resume a single Codex session, issue a solve prompt, then issue a review prompt to decide closure or escalation. It must validate that Codex updated br state after the review step and error if not.

## Goals / Non-Goals
- Goals:
  - Provide a single executable `./trudger` entrypoint.
  - Process the lowest-priority ready `trudgeable` br task each cycle.
  - Use Codex prompts stored in `~/.codex/prompts/trudge.md` and `~/.codex/prompts/trudge_review.md`.
  - Update br state based on Codex review (close, or mark requires-human).
...
- Task selection:
  - Use br readiness and priority to select the lowest-priority ready task with label `trudgeable`.
- Codex session handling:
  - Use `codex exec` for the initial call and `codex exec resume --last` for subsequent calls.
  - Render the prompt content from `~/.codex/prompts/trudge.md` and `~/.codex/prompts/trudge_review.md`, substituting the br id into `$ARGUMENTS`.
- Error handling:
  - Treat missing prompt files as a startup error with a clear message.
  - Treat missing br updates after review (no close, no requires-human) as a task failure.
