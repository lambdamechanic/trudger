...
- Goals:
  ...
  - Preserve unknown/custom top-level keys as commented YAML so they are not silently dropped.
- Non-Goals:
  - Building a full-screen TUI or editor for arbitrary YAML.
  - Auto-migrating arbitrary nested custom config structures beyond commenting out unknown top-level keys.
...
- Wizard flow:
  ...
  3. If a config exists at the target path, parse it and capture unknown/custom top-level keys.
  4. Prompt for agent template (`codex`, `claude`, `pi`), then tracking template (`br-next-task`, `bd-labels`).
  5. Build a candidate config from the selected templates plus embedded defaults for `review_loop_limit` and `log_path` (no prompts for these).
  6. If an existing config is present, compare each known config key against the candidate config:
  ...
  7. Append commented YAML for unknown/custom top-level keys to the output config, preceded by a warning comment, and also print a warning to stderr.
  ...
  10. If the target config exists, create a timestamped backup in the same directory, then overwrite-write the new config.
...
- **br-next-task** (current behavior)
  ...
  - `commands.task_update_in_progress`: `br update "$TRUDGER_TASK_ID" --status in_progress`
